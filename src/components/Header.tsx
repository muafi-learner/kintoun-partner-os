    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 sm:h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button onClick={onOpenSearch} className="p-2.5 rounded-full text-white/90 hover:text-white hover:bg-white/10 transition cursor-pointer">
            <Search className="w-5 h-5" />
          </button>
          <button onClick={onOpenNotifications} className="relative p-2.5 rounded-full text-white/90 hover:text-white hover:bg-white/10 transition cursor-pointer">
            <Bell className="w-5 h-5" />
            {unreadCount > 0 && <span className="absolute top-1.5 right-1.5 w-2.5 h-2.5 rounded-full bg-rose-500 ring-2 ring-[#00263f]"></span>}
          </button>
          <button onClick={onOpenLogin} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-bold text-white/90 hover:text-white hover:bg-white/10 border border-white/20 transition cursor-pointer">
            <LogIn className="w-3.5 h-3.5 text-amber-300" />
            <span className="hidden sm:inline">Ganti Akun</span>
          </button>
          <button onClick={onOpenProfile} className="p-1 rounded-xl hover:bg-white/10 text-white transition cursor-pointer flex items-center justify-center">
            <div className={`w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center font-bold text-xs transition-transform duration-150 hover:scale-105 shadow-xs ${
              role === 'admin' ? 'bg-amber-400 text-slate-950 ring-2 ring-amber-300/90' : 'bg-sky-600 text-white border border-sky-400/50'
            }`}>
